{ pkgs, lib, stdenv, symlinkJoin, writeText, makeWrapper }:
let config = writeText "alacritty.yml" (lib.generators.toYAML {} {
    env.TERM = "alacritty";
    key_bindings = if stdenv.isDarwin then [{
        key = "N";
        mods = "Command";
        command = {
            program = "open";
    ...
    window = {
        padding.x = 10;
        padding.y = if stdenv.isDarwin then 24 else 10;
